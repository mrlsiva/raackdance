...
<form id="contactus-form" class="contactus-form" action="{{route('registerstudentmail.store')}}" method="POST">
	...
	<div class="form-field {{ $errors->has('dob') ? 'has-error' : ''}}">                        
		<input id="dob" type="text" name="dob" class="form-input @error('dob') is-invalid @enderror"  placeholder="DOB (DD-MM-YYYY)" value="{{ old('dob') }}" maxlength="50" required autocomplete="off" />
	</div>
